// UNA PILA ES UNA LISTA Y EL UNICO METODO PARA INGRESAR DATOS ES AL INICIO
// Para eliminar datos se elimina el inicio para llegar al final
// "El ultimo en entrar es el primero en salir"
// Los elementos se van colocando hacia abajo
...
void Insertar(struct Nodo *&pila, int n);   // Aqui con el *& estoy modificando la struct
void Eliminar(struct Nodo *&pila);

// Porque el parametro de imprimir es puntero simple ?
void Imprimir(struct Nodo *pila);                         // Y aqui como no se modifica nada, se deja un solo puntero
